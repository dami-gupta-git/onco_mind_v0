    ...
    for result in results:
        ...
        if result.get("gene_name") or result.get("gene_id"):
            ...
            if result.get("ref_allele") and result.get("alt_allele"):
                lines.append(
                    f"  Alleles:        {result['ref_allele']}>{result['alt_allele']}"
                )
...
""",
    ...
    parser.add_argument("gene", nargs="?", help="Gene symbol (e.g., BRAF, EGFR)")
    parser.add_argument(
        "variant",
        nargs="?",
    ...
    parser.add_argument(
        "--stdin", "-i", action="store_true", help="Read from stdin (batch mode)"
    )
    parser.add_argument(
        "--lookup",
        "-l",
    ...
    if args.batch:
        ...
        if args.lookup:
            results = [
                asyncio.run(normalize_variant_with_lookup(args.gene, args.variant))
            ]
        else:
    ...
    if args.quiet:
        for result in results:
            if "error" in result:
                print(
                    f"ERROR: {result.get('gene', '?')} {result.get('variant_original', '?')}: {result['error']}",
                    file=sys.stderr,
                )
            else:
                ...
                print(
                    f"{result['gene']}\t{result['variant_normalized']}\t{chrom}\t{genomic}"
                )
        return
