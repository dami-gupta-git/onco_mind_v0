...
class ClinicalTrialEvidence(EvidenceItemBase):
    ...
    conditions: list[str] = Field(
        default_factory=list, description="Cancer types/conditions"
    )
    interventions: list[str] = Field(
        default_factory=list, description="Drug/treatment names"
    )
    ...
    def is_phase2_or_later(self) -> bool:
        ...
        return any(
            p in phase_upper
            for p in ["PHASE2", "PHASE3", "PHASE 2", "PHASE 3", "PHASE4", "PHASE 4"]
        )
