...
from oncomind.insight_builder.gap_detection.discordance import (
    detect_discordant_evidence_internal as _detect_discordant_evidence_internal,
)
...
class TestDiscordantEvidenceDetection:
    ...
    def test_no_conflict_when_same_source(self):
        ...
        evidence = Evidence(
            identifiers=VariantIdentifiers(
                variant_id="KRAS:G12D", gene="KRAS", variant="G12D"
            ),
            civic_evidence=[
        ...
        high_conflicts, significant_conflicts = _detect_discordant_evidence_internal(
            evidence
        )
        ...
        assert (
            len(all_conflicts) == 0
        ), f"Intra-source conflict should not be flagged: {all_conflicts}"
    ...
    def test_combination_therapy_not_flagged(self):
        ...
        assert (
            len(all_conflicts) == 0
        ), f"Combo therapy should not create conflict: {all_conflicts}"
    ...
    def test_clinvar_benign_vs_civic_actionable_conflict_variant_level(self):
        ...
        evidence = Evidence(
            identifiers=VariantIdentifiers(
                variant_id="TEST:V123M", gene="TEST", variant="V123M"
            ),
            clinvar_entries=[
                ClinVarEvidence(
    ...
    def test_clinvar_benign_gene_level_no_civic_conflict(self):
        ...
        clinvar_civic_conflicts = [
            c for c in all_conflicts if "benign" in c.lower() and "civic" in c.lower()
        ]
        assert (
            len(clinvar_civic_conflicts) == 0
    ...
    def test_clinvar_internal_conflict_variant_level(self):
        ...
        clinvar_conflicts = [
            ...
            if "clinvar" in c.lower() and "conflicting" in c.lower()
        ]
        assert (
            len(clinvar_conflicts) >= 1
        ), f"Should flag ClinVar internal conflict: {high_conflicts}"
        # Should mention variant level
        assert any(
            "variant level" in c.lower() for c in clinvar_conflicts
        ), f"Should mention 'variant level' in conflict: {high_conflicts}"
    ...
    def test_cross_source_drug_conflict_variant_level(self):
        ...
        assert (
            len(drug_conflicts) >= 1
        ), f"Should flag cross-source drug conflict: {high_conflicts}"
        # Should mention it's at variant level
        assert any(
            "variant level" in c.lower() for c in drug_conflicts
    ...
    def test_cross_source_drug_conflict_gene_level_not_flagged(self):
        ...
        drug_conflicts = [
            c
            for c in all_conflicts
            if "erlotinib" in c.lower() and "conflict" in c.lower()
        ]
        assert (
            len(drug_conflicts) == 0
    ...
    def test_fda_vs_vicc_conflict_significant_severity(self):
        ...
        fda_vicc_conflicts = [
            c
            for c in significant_conflicts
            if "fda" in c.lower() and "vicc" in c.lower()
        ]
        assert (
            len(fda_vicc_conflicts) >= 1
        ), f"Should flag FDA vs VICC conflict as SIGNIFICANT: {significant_conflicts}"
        # Should NOT be in HIGH conflicts
        fda_vicc_high = [
            c for c in high_conflicts if "fda" in c.lower() and "vicc" in c.lower()
        ]
        assert (
            len(fda_vicc_high) == 0
