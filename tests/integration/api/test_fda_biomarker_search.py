...
class TestFDABiomarkerSearchLive:
    ...
    async def test_idh1_finds_vorasidenib(self, client):
        ...
        assert (
            "VORANIGO" in brand_names or "VORASIDENIB" in generic_names
    ...
    async def test_idh1_finds_ivosidenib(self, client):
        ...
        assert (
            "TIBSOVO" in brand_names
        ), f"Expected TIBSOVO for IDH1, got: {brand_names}"
    ...
    async def test_idh1_finds_olutasidenib(self, client):
        ...
        assert (
            "REZLIDHIA" in brand_names
        ), f"Expected REZLIDHIA for IDH1, got: {brand_names}"
    ...
    async def test_idh1_biomarker_specificity_is_gene_level(self, client):
        ...
        for r in results:
            if r.brand_name and r.brand_name.upper() in [
                "VORANIGO",
                "TIBSOVO",
                "REZLIDHIA",
            ]:
                assert (
                    r.biomarker_specificity == "gene"
                ), f"{r.brand_name} should have gene-level specificity, got: {r.biomarker_specificity}"
                assert (
                    "IDH1" in r.target_genes
    ...
    async def test_braf_finds_vemurafenib(self, client):
        ...
        assert (
            "ZELBORAF" in brand_names
        ), f"Expected ZELBORAF for BRAF, got: {brand_names}"
    ...
    async def test_braf_v600e_filters_correctly(self, client):
        ...
        for r in results:
            print(
                f"  - {r.brand_name}: specificity={r.biomarker_specificity}, variants={r.specific_variants}"
            )
        ...
        for r in results:
            has_v600e = (
                any("V600E" in v.upper() for v in r.specific_variants)
                if r.specific_variants
                else False
            )
            ...
            assert (
                has_v600e or is_gene_level or has_v600_mention
    ...
    async def test_braf_variant_specific_approvals(self, client):
        ...
        assert (
            len(variant_specific_drugs) > 0
        ), "Should find variant-specific BRAF approvals"
    ...
    async def test_egfr_finds_osimertinib(self, client):
        ...
        assert (
            "TAGRISSO" in brand_names
        ), f"Expected TAGRISSO for EGFR, got: {brand_names}"
    ...
    async def test_convenience_function_search_fda_by_biomarker(self):
        ...
        assert all(
            isinstance(r, FDALabelInfo) for r in results
        ), "Should return FDALabelInfo objects"
    ...
    async def test_indications_text_captured(self, client):
        ...
        for r in results:
            if r.brand_name and r.brand_name.upper() == "VORANIGO":
                ...
                assert (
                    "IDH1" in r.indications_and_usage.upper()
                ), "Indications should mention IDH1"
                # Check for glioma indication
                assert (
                    "GLIOMA" in r.indications_and_usage.upper()
                    or "ASTROCYTOMA" in r.indications_and_usage.upper()
