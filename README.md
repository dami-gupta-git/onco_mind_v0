...
- **Rules engine for tier classification**: AMP/ASCO/CAP guidelines are fundamentally a decision tree; a deterministic rules engine applies them consistently (91% accuracy vs ~(50-80)% when LLM attempted classification directly)
...
- ~50-80% accuracy when LLM attempted tier assignment
...
|-----------|----------|-------------------------------------------------------------------------------------------|
| **Tier classification** | Rules engine | AMP/ASCO/CAP guidelines are deterministic; rules yield ~90% accuracy vs ~50-80% with LLM  |
...
# 3. Run CLI commands
oncomind assess BRAF V600E --tumor "Melanoma" --no-log
oncomind batch benchmarks/sample_batch.json --no-log
oncomind validate benchmarks/gold_standard.json --no-log
```
...
oncomind assess BRAF V600E --model claude-3-haiku-20240307

# Google Gemini
export GOOGLE_API_KEY="your-key-here"
oncomind assess BRAF V600E --model gemini/gemini-1.5-pro

# Groq LLaMA
export GROQ_API_KEY="your-key-here"
oncomind assess BRAF V600E --model groq/llama-3.1-70b-versatile
```
...
| Command | Description |
|---------|-------------|
| `oncomind assess BRAF V600E --tumor Melanoma` | Assess a single variant (research‑only tier and rationale) |
| `oncomind batch variants.json` | Process multiple variants concurrently |
| `oncomind validate benchmarks/gold_standard*.json` | Benchmark against gold‑standard datasets |
