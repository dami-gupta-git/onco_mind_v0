...
def result_to_markdown(result: dict) -> str:
    ...
    # Extract genome build from HGVS genomic if present (e.g., "GRCh38:chr7:g.123A>T")
    # Otherwise default to GRCh38 (VEP and MyVariant both use GRCh38)
    hgvs = result.get('hgvs', {})
    hgvs_genomic = hgvs.get('genomic', '')
    genome_build = "GRCh38"  # Default - VEP and MyVariant/hg38 endpoint both use GRCh38
    if hgvs_genomic and ':chr' in hgvs_genomic:
        # Extract assembly prefix (e.g., "GRCh38" from "GRCh38:chr7:g.123A>T")
        genome_build = hgvs_genomic.split(':chr')[0]
